...
export class JX {
  ...
  static async post(
    ...
    if (headers) {
      const cabecahoTipoOriginal = headers["Content-Type"]
  ...
  static async consultar(query) {
    function respostaConsulta(resposta) {
      ...
      if (valores.length) {
        valores.forEach((v) => {
          let obj = {};
          nomes.forEach((n, i) => (obj[n.name] = v[i]));
  ...
  static acionarBotao(
    ...
    function converterParametro(dadosInternos) {
      ...
      Object.keys(dadosInternos).forEach((chave) =>
  ...
  static _salvar(dados, instancia, chavePrimaria) {
    function montarDadosEnvio(
      dadosInterno,
      instanciaInterna,
      chavePrimariaInterna
    ) {
      let camposConvertidos = Object.keys(dadosInterno).reduce(
        (acumulador, chave) => ({
          ...acumulador,
          [chave.toUpperCase()]: {
      ...
      let estruturaEnvio = {
        ...
        requestBody: {
          dataSet: {
            ...
            entity: {
              fieldset: {
                list: Object.keys(dadosInterno)
                  .map((nomeCampos) => nomeCampos.toUpperCase())
                  .join(","),
              },
            },
          },
      ...
      if (chavePrimariaInterna) {
        ...
        Object.keys(chavePrimariaInterna).forEach(
          (chave) =>
  ...
  static async salvar(dados, instancia, chavesPrimarias) {
    ...
    if (
      chavesPrimarias &&
      chavesPrimarias instanceof Array &&
      chavesPrimarias.length
    ) {
   ...
   */
  static removerFrame(
    { instancia, paginaInicial, ...opcoes } = {
      instancia: "",
    ...
    new Promise((resolve) => {
      if (window.parent.document.getElementsByTagName("body").length) {
        ...
        window.parent.document.getElementsByTagName("body")[0].style.overflow =
          "hidden";
      }
      ...
      if (
        ...
        instancia = window.parent.document.querySelector(
          "div.GI-BUHVBPVC > div > div > div > div > div > table > tbody > tr > td > div"
        ).textContent;
      }
      ...
      if (instancia && instancia.length > 0) {
        JX.consultar(
          `SELECT NUGDG FROM TSIGDG WHERE TITULO = '${instancia}'`
        ).then((e) =>
          resolve({ gadGetID: "html5_z6dld", nuGdt: e[0].NUGDG, ...opcoes })
        );
      } else {
      ...
      setTimeout(() => {
        if (
          typeof window.parent.document.getElementsByClassName(
            "DashWindow"
          )[0] != "undefined"
        ) {
          ...
          setTimeout(
            () =>
              document.getElementsByClassName("popupContent").length
                ? document
                    .getElementsByClassName("popupContent")[0]
  ...
  static _converterTuplas(respostaParametros) {
    ...
    function iterarArray(array, chavePai, tuplasInternas) {
      array.forEach((elemento) =>
        iterarObjeto(elemento, chavePai, tuplasInternas)
      );
    }
    ...
    if (isListagemTotal) {
      ...
      for (const nomeParametro of parametrosAProcurar) {
        const parametro = arrayNormalizado.filter((item) => {
          ...
          return [nomeParametroEncontrado, moduloParametroEncontrado].includes(
            nomeParametro
          );
        })[0];
  ...
  static async getParametro(nomesParametros = "") {
    ...
    const isTipoNomeParametroValido =
      typeof nomesParametros === "string" || Array.isArray(nomesParametros);
    if (!isTipoNomeParametroValido) {
      throw new Error(
        "Forneça o nome dos parametros a serem buscados como Texto ou Array de Textos!"
      );
    }

    const isAlgumNomeInvalido =
      Array.isArray(nomesParametros) &&
      !nomesParametros.every(
    ...
    nomesParametros =
      Array.isArray(nomesParametros) && isListagemTotal ? "" : nomesParametros;
    const isParametroUnico =
      typeof nomesParametros === "string" || nomesParametros.length === 0;
    const parametrosAProcurar = isParametroUnico
      ? [nomesParametros]
      : nomesParametros;
    ...
    const requisicoes = parametrosAProcurar.map(async (parametro) => {
      ...
      const parametrosEncontrados =
        JX._converterTuplas(resposta.responseBody.root) || [];

      return parametrosEncontrados;
    });

    const parametros = await Promise.all(requisicoes);
    return JX._montagemSerializacaoParametros(
      parametros,
      parametrosAProcurar,
      isListagemTotal
    );
  ...
  static _formatarUrlChamadaServico(
    ...
    const aplicacaoUrlComercial = [
      `SelecaoDocumento`,
      `br.com.sankhya.mgecom.mov.selecaodedocumento`,
    ];
    const aplicacaoUrlFinanceiro = [
      `MovimentacaoFinanceira`,
      `br.com.sankhya.fin.cad.movimentacaoFinanceira`,
    ];
    const aplicacaoUrlServico = [
      `ConsultaOS`,
      `br.com.sankhya.os.mov.OrdemServico`,
    ];
  ...
  static async chamarServico(
    ...
    if (dadosAdicionais) {
      ...
      aplicacaoRequisitante =
        dadosAdicionais.aplicacao || aplicacaoRequisitante;
    ...
    const cabecalhoFinal = {
      ...cabecalhoRequisicao,
      "Content-Type": isChamadaJson
        ? "application/json; charset=UTF-8"
        : "text/xml; charset=UTF-8",
    };

    let url = JX._formatarUrlChamadaServico(
      nomeModulo,
      nomeServico,
      aplicacaoRequisitante
    );
    [url, corpoRequisicao] = JX._formatarRequisicaoChamadaServico(
      url,
      nomeServico,
      dados,
      isChamadaJson
    ...
    const dadosResposta = isChamadaJson
      ? await resposta.json()
      : await resposta.text();
